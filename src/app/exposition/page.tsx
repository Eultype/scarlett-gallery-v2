...
export default function ExpositionPage() {
    ...
    return (
        <main className="fixed top-0 left-0 w-full h-full bg-[#EAE0D5] overflow-hidden z-[9999]">
            {/* Écran d'accueil */}
            {!startLocked && !selectedArtworkData && (
                <div className="absolute top-0 left-0 w-full h-full z-50 bg-[#EAE0D5]/90 flex flex-col items-center justify-center text-gray-900 p-8">
            ...
            {startLocked && !selectedArtworkData && isMobile && (
                <>
                    <button 
                        ...
                        className="absolute top-8 right-8 z-50 bg-black/40 border border-white/20 text-white px-4 py-2 uppercase text-[10px] tracking-widest rounded-full"
                    >
                        Quitter
                    </button>
                    <VirtualJoystick />
